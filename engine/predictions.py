"""
JYOTISHA PREDICTIONS ENGINE
Dasha-wise predictions grounded in classical Shastra.
Sources: BPHS, Phaladeepika, Brihat Jataka, Saravali, Uttara Kalamrita,
         Skanda Purana, Yoga Vasishtha, Rigveda 10.85, Bhagavad Gita.

This module is intentionally separate so it can be imported cleanly
without any pyc-cache conflicts from older builds.
"""

import datetime
from .calc import SIGN_LORDS, NAKSHATRAS
from .interpret import (
    MD_THEMES, HOUSE_KARAK,
    dasha_quality_score,
    jupiter_transit_modifier,
    saturn_transit_modifier,
    compute_year_scores,
    antardasha_sequence,
    active_dasha,
)

# ── HOUSE → LIFE-AREA PREDICTIONS ────────────────────────────────────────────
HOUSE_LIFE_AREAS = {
    1: {
    ...
    8: {
        ...
        "spirituality": "Per BPHS: the 8th house is Ayus Sthana (longevity) and the gateway to occult knowledge. Maha Mrityunjaya Japa, Kali worship, and deep Dhyana practice yield profound transformation. Do not fear this period — it is the forge of the soul.",
    },
    9: {
        ...
        "finances":     "Fortune through dharmic action, foreign sources, higher education, and grace. Per Phaladeepika: the 9th house dasha is 'Bhagya Udaya' — the rising of fortune stored in past-life Punya.",
    ...
    12: {
        ...
        "spirituality": "Per BPHS: the 12th house is Moksha Sthana — the house of liberation. This period is an extraordinary invitation to deep Dhyana, ashram retreat, and moksha-oriented practice. Vipareet Raja Yoga may activate, transforming apparent worldly loss into hidden, profound inner blessing.",
    },
}
...
def _dignity_qualifier(dignity: str) -> tuple:
    if "Exalted" in dignity:
        ...
        return ("challenging yet transformative",
                "Per Brihat Jataka: a debilitated dasha lord activates the potential for Neecha Bhanga (cancellation of debility). The challenge contains the seed of its own reversal. The native who persists with dharmic conduct emerges stronger.")
    else:
        return ("moderate",
...
def _ad_prediction_text(md_lord: str, ad_lord: str, ad_house: int,
                         ad_dignity: str, score: float) -> str:
    ...
    if score >= 9.0:
        return (f"{ad_lord} Antardasha brings its themes of {theme} into this {md_lord} period with "
                f"{quality_word} force, powerfully illuminating {house_theme}. Per Phaladeepika: when both MD and AD lords are strong, "
        ...
        return (f"{ad_lord} Antardasha is one of the more demanding sub-periods in this Maha Dasha, requiring patience, "
                ...
                f"it is the time for consolidation, deep spiritual practice, and dharmic conduct in all dealings. "
                f"You are not being punished — you are being refined.")


def _generate_md_guidance(lord: str, avg_score: float) -> dict:
    MANTRA = {
    ...
    for md in dasha_seq:
        ...
        predictions.append({
            ...
            "shastra_ref":  MD_SHASTRA_REFS.get(lord,
                f"Per BPHS: {lord} Maha Dasha activates the life domains governed by {lord}'s "
                f"placement in the natal chart."),
            "guidance":     _generate_md_guidance(lord, avg_score),
